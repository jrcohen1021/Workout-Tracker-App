...
const TABS = [
  { id: "workouts", label: "Workouts", icon: Dumbbell, accent: "#34d399" },
...
export default function WorkoutFoodApp() {
  ...
  const activeAccent = TABS.find((t) => t.id === activeTab)?.accent || "#34d399";
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col max-w-md mx-auto relative">
      <header className="px-4 pt-5 pb-3 sticky top-0 bg-zinc-950/95 backdrop-blur z-20 border-b border-zinc-900">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="text-[11px] text-zinc-500 border border-zinc-800 rounded-full px-2.5 py-1 active:text-zinc-300"
            >
              Reload
            </button>
            <button
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-zinc-900 border-t border-zinc-800 flex z-30">
        {TABS.map((t) => {
          const Icon = t.icon;
          ...
          return (
            ...
            >
              <Icon size={22} color={active ? t.accent : "#71717a"} strokeWidth={active ? 2.5 : 2} />
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button
          onClick={startNew}
          className="flex-1 py-3.5 rounded-2xl bg-emerald-400 text-zinc-950 font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition"
        >
          <Plus size={20} strokeWidth={2.5} /> New Workout
        </button>
        <button
          onClick={() => importInputRef.current?.click()}
          className="px-4 rounded-2xl bg-zinc-900 border border-zinc-800 text-zinc-300 font-medium text-sm active:scale-[0.98] transition"
        >
          Import
        </button>
      ...
      {sessions.length > 0 && !confirmClearAll && (
        <button onClick={() => setConfirmClearAll(true)} className="text-xs text-zinc-600 active:text-red-400">
          Clear all workouts
        </button>
      )}
      {confirmClearAll && (
        <div className="bg-zinc-900 border border-red-500/30 rounded-xl p-3.5 space-y-2.5">
          ...
          <div className="flex gap-2">
            <button onClick={() => setConfirmClearAll(false)} className="flex-1 py-2.5 rounded-lg bg-zinc-800 text-zinc-300 text-sm font-medium">
              Cancel
            </button>
            <button
      ...
      {importFile && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 space-y-2.5">
          <p className="font-medium text-sm text-zinc-100">Import Preview</p>
          <p className="text-xs text-zinc-400">
            {importFile.exercises.length} exercises · {importFile.sessions.length} workout sessions found in this file.
          </p>
          <p className="text-xs text-zinc-500">Anything already in your library or history (matched by id) will be skipped, so it's safe to import the same file twice.</p>
          <div className="flex gap-2 pt-1">
            <button onClick={() => setImportFile(null)} className="flex-1 py-2.5 rounded-lg bg-zinc-800 text-zinc-300 text-sm font-medium">
              Cancel
            </button>
            <button onClick={confirmImport} className="flex-1 py-2.5 rounded-lg bg-emerald-400 text-zinc-950 text-sm font-semibold">
              Import
            </button>
          </div>
        </div>
      ...
      {sessions.length === 0 && (
        <p className="text-zinc-500 text-sm text-center pt-10">No workouts logged yet. Start one above.</p>
      )}

      {dates.map((date) => (
        <div key={date}>
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">{fmtDate(date)}</p>
            ...
            {grouped[date].map((s) => {
              ...
              return (
                <div key={s.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                  ...
                  >
                    <div>
                      <p className="font-medium text-zinc-100">{s.name || "Workout"}</p>
                      <p className="text-xs text-zinc-500">{s.exercises.length} exercises · {totalSets} sets</p>
                    </div>
                  ...
                  {isOpen && (
                    <div className="px-4 pb-4 border-t border-zinc-800 pt-3 space-y-3">
                      {s.exercises.map((ex, i) => (
                        <div key={i}>
                          <p className="text-sm font-medium text-zinc-200">{ex.exerciseName}</p>
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          onClick={() => startEdit(s)}
                          className="flex-1 py-2 rounded-lg bg-zinc-800 text-zinc-200 text-sm font-medium flex items-center justify-center gap-1.5"
                        >
                          <Pencil size={14} /> Edit
                        </button>
                        {confirmDeleteId === s.id ? (
                          ...
                          <button
                            ...
                            className="flex-1 py-2 rounded-lg bg-zinc-800 text-red-400 text-sm font-medium flex items-center justify-center gap-1.5"
                          >
    ...
    <div className="space-y-4 pb-4">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm placeholder-zinc-600 outline-none focus:border-emerald-400"
        />
        <input
          type="date"
          value={draft.date}
          onChange={(e) => setDraft({ ...draft, date: e.target.value })}
          className="bg-zinc-900 border border-zinc-800 rounded-xl px-2 py-2.5 text-sm text-zinc-300 outline-none focus:border-emerald-400"
        />
      </div>

      {draft.exercises.map((ex, exIdx) => (
        <div key={exIdx} className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5">
          <div className="flex items-center justify-between mb-2.5">
            <p className="font-medium text-zinc-100">{ex.exerciseName}</p>
            <button onClick={() => removeExercise(exIdx)} className="p-1 text-zinc-500 active:text-red-400">
              <X size={18} />
            </button>
          </div>
          <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 mb-1.5 px-1">
            <span className="text-[11px] text-zinc-500 uppercase">Set</span>
            <span className="text-[11px] text-zinc-500 uppercase">Lbs</span>
            <span className="text-[11px] text-zinc-500 uppercase">Reps</span>
            <span></span>
          </div>
          {ex.sets.map((st, setIdx) => (
            <div key={setIdx} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 mb-2 items-center">
              <span className="text-sm text-zinc-400 pl-1">{setIdx + 1}</span>
              <input
                type="number"
                inputMode="decimal"
                value={st.weight}
                onChange={(e) => updateSet(exIdx, setIdx, "weight", e.target.value)}
                className="bg-zinc-800 rounded-lg px-2 py-2.5 text-center text-base outline-none focus:ring-1 focus:ring-emerald-400"
              />
              <input
                type="number"
                inputMode="numeric"
                ...
                className="bg-zinc-800 rounded-lg px-2 py-2.5 text-center text-base outline-none focus:ring-1 focus:ring-emerald-400"
              />
              <button onClick={() => removeSet(exIdx, setIdx)} className="p-2 text-zinc-600 active:text-red-400">
                <X size={16} />
              </button>
            </div>
          ))}
          <button
            ...
            className="w-full mt-1 py-2 rounded-lg bg-zinc-800 text-emerald-400 text-sm font-medium flex items-center justify-center gap-1.5"
          >
      ...
      {!pickerOpen ? (
        <button
          ...
          className="w-full py-3 rounded-xl border-2 border-dashed border-zinc-700 text-zinc-400 font-medium flex items-center justify-center gap-2 active:bg-zinc-900"
        >
      ...
      <div className="flex gap-2 pt-2">
        <button onClick={onCancel} className="flex-1 py-3 rounded-xl bg-zinc-800 text-zinc-300 font-medium">
          Cancel
        </button>
        <button
          onClick={onSave}
          className="flex-1 py-3 rounded-xl bg-emerald-400 text-zinc-950 font-semibold flex items-center justify-center gap-1.5"
        >
...
function ExercisePicker({ exercises, setExercises, onPick, onClose }) {
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 space-y-3">
        ...
        <div className="space-y-2">
          {Object.entries(MUSCLE_TAXONOMY).map(([group, regions]) => (
            <div key={group}>
              <p className="text-[11px] text-zinc-500 uppercase mb-1">{group}</p>
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={() => setPendingMuscles(null)} className="flex-1 py-2.5 rounded-lg bg-zinc-800 text-zinc-300 text-sm font-medium">
            Back
          </button>
          <button onClick={confirmNewExercise} className="flex-1 py-2.5 rounded-lg bg-emerald-400 text-zinc-950 text-sm font-semibold">
            Add Exercise
          </button>
        </div>
      </div>
  ...
  if (showForm) {
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 space-y-2.5">
        <div className="flex items-center justify-between">
          <p className="font-medium text-zinc-100 text-sm">New Exercise</p>
          <button onClick={() => setShowForm(false)} className="p-1 text-zinc-500"><X size={18} /></button>
        </div>
        <div>
          <p className="text-[11px] text-zinc-500 mb-1">Movement name</p>
          <input
            autoFocus
            value={newBase}
            onChange={(e) => setNewBase(e.target.value)}
            placeholder="e.g. Chest Press"
            className="w-full bg-zinc-800 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-emerald-400"
          />
        </div>
        <div>
          <p className="text-[11px] text-zinc-500 mb-1">Equipment / brand (optional)</p>
          <input
            value={newEquipment}
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 space-y-2.5">
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-zinc-800 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-emerald-400"
        />
      ...
      <div className="max-h-52 overflow-y-auto space-y-1">
        {filtered.map((e) => (
          <button
            key={e.id}
            onClick={() => onPick(e)}
            className="w-full text-left px-3 py-2.5 rounded-lg bg-zinc-800 active:bg-zinc-700 text-sm"
          >
            {e.name}
            {e.muscles?.length > 0 && (
              <span className="text-zinc-500 text-xs"> · {e.muscles.map((m) => m.region).join(", ")}</span>
            )}
          </button>
        ))}
      </div>
...
function ProgressTab({ exercises, sessions }) {
  ...
  if (loggedExercises.length === 0) {
    return <p className="text-zinc-500 text-sm text-center pt-10">Log a workout to start seeing progress.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {variants.length > 1 && (
        <div className="flex gap-1.5 overflow-x-auto pb-0.5">
          <button
            ...
            style={
              selectedVariant === "all"
                ? { backgroundColor: "#38bdf833", borderColor: "#38bdf8", color: "#38bdf8" }
                : { borderColor: "#3f3f46", color: "#a1a1aa" }
            }
          >
      ...
      <div className="flex gap-2">
        {[{ id: "weight", label: "Top Set Weight" }, { id: "volume", label: "Volume" }].map((m) => (
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium border ${metric === m.id ? "bg-sky-400 text-zinc-950 border-sky-400" : "bg-zinc-900 border-zinc-800 text-zinc-400"}`}
          >
            {m.label}
          </button>
        ))}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={points} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey={metric === "weight" ? "topWeight" : "volume"} stroke="#38bdf8" strokeWidth={2.5} dot={{ r: 3, fill: "#38bdf8" }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function FoodTab({ foodLog, setFoodLog, targets, setTargets }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => shiftDay(-1)} className="p-2 text-zinc-400"><ChevronLeft size={20} /></button>
        <p className="font-medium text-sm">{date === todayStr() ? "Today" : fmtDate(date)}</p>
        <button onClick={() => shiftDay(1)} disabled={date === todayStr()} className="p-2 text-zinc-400 disabled:opacity-30"><ChevronRight size={20} /></button>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-zinc-300">Today's Targets</p>
          <button onClick={() => setShowSettings(!showSettings)} className="p-1.5 text-zinc-500"><Settings size={16} /></button>
        </div>
        {showSettings ? (
      ...
      <button
        ...
        className="w-full py-3.5 rounded-2xl bg-amber-400 text-zinc-950 font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition"
      >
      ...
      <div className="space-y-2">
        {dayEntries.length === 0 && <p className="text-zinc-500 text-sm text-center pt-6">No meals logged for this day.</p>}
        {dayEntries.map((e) => (
          <div key={e.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 flex items-start justify-between">
            <div>
              <p className="font-medium text-sm text-zinc-100">{e.name}</p>
              <p className="text-xs text-zinc-500 mt-0.5">{e.time} · {e.calories} kcal · P{e.protein} C{e.carbs} F{e.fat}</p>
            </div>
            <button onClick={() => deleteEntry(e.id)} className="p-1.5 text-zinc-600 active:text-red-400"><Trash2 size={16} /></button>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex justify-between text-xs mb-1">
        ...
        <span className="text-zinc-300 font-medium">{Math.round(value)} / {target} {unit}</span>
      </div>
      ...
      {[
        ...
        <div key={f.key} className="flex items-center justify-between">
          ...
          <input
            ...
            className="w-24 bg-zinc-800 rounded-lg px-2 py-1.5 text-right text-sm outline-none focus:ring-1 focus:ring-amber-400"
          />
        </div>
      ))}
      <button onClick={() => onSave(t)} className="w-full py-2.5 rounded-lg bg-amber-400 text-zinc-950 text-sm font-semibold mt-1">
        Save Targets
      </button>
    </div>
  );
...
function AddMealSheet({ onAdd, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/60" onClick={onClose}>
      <div className="bg-zinc-900 border-t border-zinc-800 rounded-t-2xl w-full max-w-md p-4 space-y-3" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full bg-zinc-800 rounded-lg px-2.5 py-2 text-sm outline-none focus:ring-1 focus:ring-amber-400"
      />
    </div>
  );
}
...
function CardioTab({ cardioLog, setCardioLog }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        onClick={() => setShowAdd(true)}
        className="w-full py-3.5 rounded-2xl bg-rose-400 text-zinc-950 font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition"
      >
          ...
          <div className="flex gap-2">
            {typesLogged.map((t) => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-medium border ${chartType === t ? "bg-rose-400 text-zinc-950 border-rose-400" : "bg-zinc-900 border-zinc-800 text-zinc-400"}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            {[{ id: "pace", label: "Pace" }, { id: "distance", label: "Distance" }, { id: "elevation", label: "Elevation" }].map((m) => (
              <button
                key={m.id}
                onClick={() => setMetric(m.id)}
                className={`flex-1 py-1.5 rounded-lg text-xs font-medium border ${metric === m.id ? "bg-zinc-800 border-rose-400 text-rose-400" : "bg-zinc-900 border-zinc-800 text-zinc-500"}`}
              >
                {m.label}
              </button>
            ))}
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 h-56">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="space-y-2">
        {sorted.length === 0 && <p className="text-zinc-500 text-sm text-center pt-6">No runs or hikes logged yet.</p>}
        {sorted.map((a) => {
          const Icon = a.type === "Hike" ? Mountain : Footprints;
          return (
            <div key={a.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <p className="font-medium text-sm text-zinc-100">{a.name || a.type}</p>
                    <p className="text-xs text-zinc-500">{fmtDate(a.date)}</p>
                  </div>
                </div>
                {confirmDeleteId === a.id ? (
                  <button onClick={() => deleteActivity(a.id)} className="text-xs text-red-400 font-medium px-2 py-1">Confirm</button>
                ) : (
                  <button onClick={() => setConfirmDeleteId(a.id)} className="p-1.5 text-zinc-600 active:text-red-400"><Trash2 size={16} /></button>
                )}
              </div>
              <div className="grid grid-cols-4 gap-1 mt-2.5 pt-2.5 border-t border-zinc-800 text-center">
                <div><p className="text-sm font-semibold text-zinc-200">{Number(a.distance).toFixed(2)}</p><p className="text-[10px] text-zinc-500">miles</p></div>
                <div><p className="text-sm font-semibold text-zinc-200">{formatDuration(a.duration)}</p><p className="text-[10px] text-zinc-500">time</p></div>
                <div><p className="text-sm font-semibold text-zinc-200">{formatPace(a.duration, a.distance)}</p><p className="text-[10px] text-zinc-500">pace</p></div>
                <div><p className="text-sm font-semibold text-zinc-200">{Math.round(a.elevationGain || 0)}</p><p className="text-[10px] text-zinc-500">ft gain</p></div>
              </div>
            </div>
          );
        })}
...
function AddCardioSheet({ onAdd, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/60" onClick={onClose}>
      <div className="bg-zinc-900 border-t border-zinc-800 rounded-t-2xl w-full max-w-md p-4 space-y-3 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
